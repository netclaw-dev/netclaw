...
| Tool | Purpose |
|------|---------|
| `set_reminder` | Schedule or update (upsert) one-shot, interval, or cron reminders by ID |
...
Key parameters: `id` (stable reminder identifier, kebab-case slug -- if a
reminder with this ID exists it will be updated instead of creating a duplicate),
`name` (human-readable title), `prompt` (instructions to execute),
...
The `id` parameter enables idempotent scheduling: calling `set_reminder` with the
same ID updates the existing reminder instead of creating a duplicate. Use
descriptive IDs like `"ram-pricing-daily"` or `"weekly-standup-prep"`.
...
| `netclaw reminder create <id> <type> <schedule> "<prompt>"` | Create or update a reminder by ID |
