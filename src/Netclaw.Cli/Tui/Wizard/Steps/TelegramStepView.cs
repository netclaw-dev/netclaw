// -----------------------------------------------------------------------
// <copyright file="TelegramStepView.cs" company="Petabridge, LLC">
...
public sealed class TelegramStepView : IWizardStepView
{
    ...
    {
        var vm = (TelegramStepViewModel)stepVm;
        _vm = vm;
        ...
        return vm.CurrentSubStep switch
        {
            ...
            2 => BuildMentionOnlySubStep(vm, callbacks),
    ...
    private ILayoutNode BuildEnableSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
        var yesLabel = "Yes - configure Telegram bot";
        ...
        _enabledList.SelectionConfirmed
            .Subscribe(selected =>
            {
                ...
                vm.TelegramEnabled = selected[0] == yesLabel;
                callbacks.AdvanceStep();
            })
            .DisposeWith(callbacks.Subscriptions);

        return Layouts.Vertical()
            .WithChild(new TextNode("  Enable Telegram integration?").WithForeground(Color.White))
            .WithChild(_enabledList);
    }

    private ILayoutNode BuildBotTokenSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
        _botTokenInput = new TextInputNode()
            .AsPassword()
            .WithPlaceholder("Telegram bot token");
        ...
        _botTokenInput.Submitted
            .Subscribe(text =>
            {
                if (string.IsNullOrWhiteSpace(text))
                {
                    ...
                    {
                        callbacks.ShowValidationError(ChannelsEditorValidationMessages.TelegramBotTokenRequired);
                    }
        ...
        var layout = Layouts.Vertical()
            .WithChild(new TextNode("  Telegram Bot Token:").WithForeground(Color.White))
    ...
    private ILayoutNode BuildChannelIdsSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
    ...
    private ILayoutNode BuildMentionOnlySubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
        const string yes = "Yes - require a mention or reply in groups";
        const string no = "No - accept every allowed group message";
        _mentionOnlyList = Layouts.SelectionList(yes, no).WithMode(SelectionMode.Single).WithHighlightColors(Color.Black, Color.Cyan);
        _mentionOnlyList.OnFocused();
        _lastFocusedList = _mentionOnlyList;
        _lastFocusedInput = null;
        _mentionOnlyList.SelectionConfirmed.Subscribe(selected =>
        {
            if (selected.Count == 0) return;
            vm.MentionOnly = selected[0] == yes;
            callbacks.AdvanceStep();
        }).DisposeWith(callbacks.Subscriptions);
        return Layouts.Vertical().WithChild(new TextNode("  Require a mention in groups?").WithForeground(Color.White)).WithChild(_mentionOnlyList);
    }

    private ILayoutNode BuildDmEnabledSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
    ...
    private ILayoutNode BuildUserAccessChoiceSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
    ...
    private ILayoutNode BuildAllowedUserIdsSubStep(TelegramStepViewModel vm, StepViewCallbacks callbacks)
    {
        _allowedUserIdsInput = new TextInputNode()
            .WithPlaceholder("129847561203948576, 130111223344556677  (Telegram user IDs)");
