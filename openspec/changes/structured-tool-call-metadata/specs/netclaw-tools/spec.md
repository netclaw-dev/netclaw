...
Execution SHALL enforce a per-call timeout: the LLM MAY request a timeout via
the `_timeout_seconds` meta field, clamped between `ToolConfig.ShellTimeoutSeconds`
(floor, default 60s) and `ToolConfig.MaxToolTimeoutSeconds` (ceiling, default
600s). When no hint is provided, the default `ShellTimeoutSeconds` applies.
Output SHALL be truncated to a configurable limit. Before execution, the shell
tool SHALL check the hard deny list via `ShellCommandPolicy`. Hard-denied
commands SHALL be rejected before `ToolPathPolicy` path checks.
...
#### Scenario: Execution timeout enforced with default

- **GIVEN** a shell command is running with no `_timeout_seconds` meta
...
#### Scenario: Execution timeout enforced with LLM hint

- **GIVEN** a shell command is running with `_timeout_seconds: 300`
- **AND** `MaxToolTimeoutSeconds` is 600
- **WHEN** the command exceeds 300 seconds
- **THEN** the process is terminated
- **AND** the tool returns a timeout error message indicating the requested
  timeout was reached
...
name, invoking session, timestamp, allow/deny/approval result, approval
decision details when applicable, rationale (from `ToolCallMeta`), and
timeout hint (from `ToolCallMeta`). The `ToolAccessDecision` SHALL support
three outcomes: `Allow`, `Deny(reason)`, and `RequiresApproval(context)`.
...
- **AND** an audit record is logged with tool name, session ID, timestamp,
  `allow` result, rationale, and timeout hint
...
- **THEN** audit records show tool name, invoking session, timestamp,
  allow/deny/approval result, rationale, and timeout hint for each invocation
