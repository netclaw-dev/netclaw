...
When building the compaction prompt from conversation history, tool calls
with `ToolCallMeta` containing a rationale SHALL be rendered using the
rationale as the primary representation:
```
→ {tool_name}: "{rationale}"
```
instead of the current raw-arguments format:
```
[Called tool: {tool_name}({arguments_json})]
```
Tool calls without meta (legacy messages) SHALL continue to use the current
raw-arguments format. This rationale-based rendering provides high-signal
breadcrumbs for session resumption and reduces token waste from verbose
argument JSON in the compaction prompt.
...
#### Scenario: Tool calls rendered with rationale in compaction prompt

- **GIVEN** conversation history contains tool calls with `ToolCallMeta`
  rationale
- **WHEN** `CompactionPromptBuilder` builds the compaction input
- **THEN** tool calls with rationale are rendered as
  `→ {tool_name}: "{rationale}"`
- **AND** raw arguments JSON is omitted from the compaction prompt

#### Scenario: Legacy tool calls rendered with raw arguments

- **GIVEN** conversation history contains tool calls without `ToolCallMeta`
  (persisted before this change)
- **WHEN** `CompactionPromptBuilder` builds the compaction input
- **THEN** legacy tool calls are rendered as
  `[Called tool: {tool_name}({arguments_json})]`
