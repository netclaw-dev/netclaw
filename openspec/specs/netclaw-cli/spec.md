...
warn when approval mode is enabled for a tool on an audience but shell is
disabled. It SHALL warn when `tool-approvals.json` contains patterns for
audiences or tools that are no longer configured.

#### Scenario: Doctor warns about approval mismatch with shell off

- **GIVEN** `shell_execute` is in Approval mode for Personal
- **AND** `ShellMode` is Off
- **WHEN** `netclaw doctor` runs
- **THEN** it emits a warning: "shell_execute in Approval mode but ShellMode
  is Off — approval config has no effect."
