# netclaw-search Specification

## Purpose

Define the search backend abstraction layer for Netclaw. Provides a pluggable
`ISearchBackend` interface with implementations for DuckDuckGo (default),
Brave Search API, and SearXNG. The search backend is selected via configuration
and injected into the web search tool at startup.
