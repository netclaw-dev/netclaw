...
internal sealed class DiscordSessionBindingActor : ReceivePersistentActor, IWithTimers
{
    ...
    private async Task HandleTrustedReminderAsync(DeliverTrustedSessionTurn message)
    {
        ...
        if (writer is null)
        {
            ...
            ackTarget.Tell(CommandDeferred.For(_sessionId, "Discord session pipeline is not initialized."));
            return;
        }
        ...
        catch (OperationCanceledException)
        {
            ...
            ackTarget.Tell(CommandDeferred.For(_sessionId, "The Discord pipeline enqueue timed out."));
        }
        catch (ChannelClosedException)
        {
            _log.Warning("Input queue closed; deferring CurrentSession reminder for session {0}", _sessionId.Value);
            RemoveReminderDeliveryObserver(message.Source.ReminderId);
            ackTarget.Tell(CommandDeferred.For(_sessionId, "The Discord pipeline input queue is closed."));
        }
    }
