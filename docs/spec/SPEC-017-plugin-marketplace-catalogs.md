...
Website issue [#121](https://github.com/netclaw-dev/netclaw-website/issues/121) tracks operator documentation.
...
Package import selects root `plugin.json` when that file exists.
It uses a host compatibility manifest only when the root file is absent.
...
3. Add a catalog participant and catalog scopes to the existing scoped sync actor.
