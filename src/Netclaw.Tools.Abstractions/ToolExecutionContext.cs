...
public sealed class ToolExecutionContext
{
    ...
    /// <summary>
    /// Sub-agent execution wiring (a <c>SubAgentExecutionWiring</c>, typed as
    /// <see cref="object"/> to keep Akka and session internals out of the tools
    /// abstraction layer): the heartbeat sink, the parent watchdog operation id,
    /// and the inactivity budgets a spawned sub-agent inherits. Wired by
    /// <c>LlmSessionActor</c>; null when there is no session.
    /// </summary>
    public object? SubAgentWiring { get; set; }
