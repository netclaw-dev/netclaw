...
internal sealed class SlackThreadBindingActor : ReceivePersistentActor, IWithTimers
{
    ...
    /// <summary>
    /// Mode B reminder re-entry delivery handler. Validates the session
    /// binding matches, builds a <see cref="ChannelInput"/> carrying the
    /// reminder content + the <see cref="MessageSource.AckTarget"/> set to
    /// <c>Sender</c> (the dispatcher's Ask temp actor preserved via the
    /// gateway's <c>Forward</c> chain), and writes it to the pipeline
    /// input queue. No ACL call and no prompt-injection scan — the
    /// reminder's audience was validated at mint time and the content is
    /// generated by the local agent itself.
    /// </summary>
