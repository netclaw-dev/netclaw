...
- Skills: `ralph-loop`, `testing-strategy`, `ui-smoke-validation` (if present)
...
- Did they start Aspire (`aspire run` in Commands Run)?
...
**Note:** We do NOT want Playwright test code (.cs files). We want evidence of
...
- If UI impacted, did it consult `ui-smoke-validation` (if available) or equivalent?
