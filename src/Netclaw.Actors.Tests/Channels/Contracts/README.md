...
| `TeamsPersonalRoutingTests` | 73 | Yes | Teams transport and persistence coverage: native card replay/expiry, route/thread identity, SDK boundary, terminal-card rendering, and focused feedback-recovery scenarios. Teams history hydration is N/A because no authenticated ordered history source exists. |
