...
- [ ] 1.1 Add strict variants to `ToolArgumentHelper` (`GetIntStrict`,
...
- [ ] 1.2 Unit tests for the strict helpers: absent → null/default, valid
...
- [ ] 2.1 Update `NetclawToolGenerator.ParseArguments` emission to call the
...
- [ ] 2.2 Snapshot/golden tests for the generator output covering each
...
- [ ] 3.1 Compute and cache the recognized-key set per native tool in
...
- [ ] 3.2 Implement rejection: unrecognized key(s) → return tool-result error
      (do not execute) naming each key, stating the tool was NOT executed, and
      listing valid argument names.
- [ ] 3.3 Implement suggestion generation (suggestion text ONLY — never
...
- [ ] 3.4 Unit tests: `TimeoutSeconds` → rejected with `_timeout_seconds`
...
- [ ] 3.5 Audit all native tools for intentionally free-form argument surfaces;
...
- [ ] 4.1 Extend `ToolCallMetaExtractor.Extract` (pipeline-side; persisted
...
- [ ] 4.2 Change `ComputeEffectiveTimeout` to report when the effective value
      differs from the requested value (ceiling clamp, below-floor); plumb as
      a notice, not a silent return.
- [ ] 4.3 Add `Notices` accumulation to `ToolExecutionContext` and append
...
- [ ] 4.4 Unit tests: 1200s request with 600s ceiling → executes at 600s AND
...
- [ ] 5.1 In `OpenAiCompatibleChatClient`, on `TryDeserializeArguments`
      ...
      arguments.
- [ ] 5.2 In `SessionToolExecutionPipeline`, detect the sentinel before meta
      ...
      executed.").
- [ ] 5.3 Tests: truncated args JSON → error result for the call id, no tool
      invocation; sentinel round-trips persistence and re-drives to the same
      rejection deterministically.

## 6. In-tool fixes

- [ ] 6.1 `WebFetchTool`: validate `Format ∈ {absent, "raw", "text"}`; reject
...
- [ ] 7.2 Verify no `Netclaw.Configuration` `*Config` property changed (no
      schema sync needed) — confirm in PR description.

## 8. Quality gates and verification

- [ ] 8.1 `dotnet slopwatch analyze` — no new violations.
- [ ] 8.2 `./scripts/Add-FileHeaders.ps1 -Verify` — headers on any new files.
- [ ] 8.3 Run the eval suite (`./evals/run-evals.sh`) — tool definitions
      changed; add/adjust an eval case asserting the model recovers from an
      unknown-key rejection in one round-trip.
- [ ] 8.4 Replay regression: drive the recorded arg shapes from session
...
- [ ] 8.5 Update `SILENT_FALLBACK_AUDIT.md` rows fixed by this change with
