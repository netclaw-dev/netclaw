...
public static class SecretsFileWriter
{
    ...
    /// <summary>
    /// Walk a JSON tree and decrypt all <c>ENC:</c> string leaf values back to plaintext.
    /// Already-plaintext values are left untouched (idempotent).
    /// </summary>
    private static string DecryptJsonLeaves(string json, ISecretsProtector protector)
    {
        var node = JsonNode.Parse(json);
        if (node is null)
            return json;
