...
Add a `<!-- triggers: ... -->` HTML comment to declare activation conditions.
These tell the agent **when** to load your skill (vs. the description which
says **what** it does).

Format: `<!-- triggers: condition1 | condition2 | condition3 -->`

- Use pipe (`|`) to separate conditions
- Keep each condition to 2-5 words
- Focus on observable situations, not abstract categories
...
<!-- triggers: deploy to staging | deploy to production | smoke test failure | rollback needed -->
```

Triggers appear in the skill index as `LOAD WHEN:` lines, giving the agent
clear signals for when to `file_read` the full skill.
