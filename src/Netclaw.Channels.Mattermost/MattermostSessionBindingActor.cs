...
internal sealed class MattermostSessionBindingActor : ReceivePersistentActor, IWithTimers
{
    ...
    private async Task HandleTrustedReminderAsync(DeliverTrustedSessionTurn message)
    {
        ...
        if (writer is null)
        {
            _log.Warning("Input queue is not initialized; deferring CurrentSession reminder");
            ackTarget.Tell(CommandDeferred.For(_sessionId, "Mattermost session pipeline is not initialized."));
            return;
        }
        ...
        catch (OperationCanceledException)
        {
            ...
            ackTarget.Tell(CommandDeferred.For(_sessionId, "The Mattermost pipeline enqueue timed out."));
        }
        catch (ChannelClosedException)
        {
            _log.Warning("Input queue closed; deferring CurrentSession reminder for session {0}", _sessionId.Value);
            RemoveReminderDeliveryObserver(message.Source.ReminderId);
            ackTarget.Tell(CommandDeferred.For(_sessionId, "The Mattermost pipeline input queue is closed."));
        }
    }
