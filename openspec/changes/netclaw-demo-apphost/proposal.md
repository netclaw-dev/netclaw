...
- **New** `samples/` top-level folder and `samples/Netclaw.Demo.AppHost` Aspire AppHost project that orchestrates Mattermost (containerized, `mattermost-preview`), Ollama (containerized via `CommunityToolkit.Aspire.Hosting.Ollama` with `qwen3:4b` model pull), and the NetClaw daemon (host process via `AddProject<>`).
...
- **New** `samples/Netclaw.Demo.Bootstrap` executable resource (preferred) or AppHost lifecycle hook that runs the Mattermost seeding sequence post-startup and emits the credentials as Aspire resource outputs consumed by the daemon's `WithEnvironment(...)` chain.
...
**New code:**
- `samples/Netclaw.Demo.AppHost/` (AppHost + README + appsettings + seeded `netclaw.json`)
- `samples/Netclaw.Demo.Bootstrap/` (executable seeding resource)
...
**Operational impact:**
- Cold first-run: pulls `mattermost-preview` image (~600MB) and `qwen3:4b` (~3GB) — documented in README with expected timings.
...
- Seeded `netclaw.json` ACL/grants must allow interesting tool calls without bypassing real `Netclaw.Security` policy gates; design-phase task to specify the exact grant set.
