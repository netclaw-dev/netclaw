...
- [ ] 5.1 Emit `ToolResultOutput` for every tool result in `LlmSessionActor.ToolExecutionCompleted` so webhook/reminder execution actors can correctly track notification-tool completion (fixes false-positive "no notification tool was invoked" warning — netclaw-dev/netclaw#546). Update existing session integration tests to drain the new output between `ToolCallOutput` and following assertions.
...
- [ ] 5.5 Add `DaemonStats.Webhooks` to the stats contract, wire it through `DaemonStatsService`, and render a `webhooks:` section in `netclaw stats` text output, TUI, and help text (netclaw-dev/netclaw#543).
