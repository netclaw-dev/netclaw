...
public sealed class ChannelPickerStepViewModelTests : WizardStepTestBase
{
    ...
    {
        using var picker = new ChannelPickerStepViewModel(_fakeProbe, _fakeDiscordProbe, _fakeMattermostProbe, _fakeTelegramProbe);
    ...
    [Fact]
    public void Adapters_IncludeMattermostAndTelegram()
    {
        using var picker = new ChannelPickerStepViewModel(_fakeProbe, _fakeDiscordProbe, _fakeMattermostProbe, _fakeTelegramProbe);
        ...
        Assert.Contains(picker.Adapters, a => a.Type == ChannelType.Telegram);
    }
