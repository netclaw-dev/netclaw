...
LIGHT_TAPES=(help init-wizard init-existing init-wizard-reverse-proxy provider-add provider-rename config-search config-exposure config-posture config-features config-audience config-channels config-surfaces config-ops-surfaces tui-cleanup mcp-permissions approvals model-manager sessions-tui)
